...
public class BaseProxy {
    ...
    protected HttpHeaders initHeaderFormDataV2() {
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.MULTIPART_FORM_DATA);
        return headers;
    }
    ...
    protected <T> T postMultiValueMap(String api, HttpHeaders headers, MultiValueMap<Object, Object> multiValueMap, Class<T> clazz) {
        HttpEntity<?> entity = new HttpEntity<>(multiValueMap, headers);
        ResponseEntity<T> response = restTemplate.exchange(api, HttpMethod.POST, entity, clazz);
        return response.getBody();
    }

    protected <T> T postMap(String api, HttpHeaders headers, Map<Object, Object> maps, Class<T> clazz) {
